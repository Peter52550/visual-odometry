...
bool testRGBD(const std::vector<std::string>& inputRGBPaths,
  ...
  namedWindow("RGBD Color", WINDOW_AUTOSIZE);  // 窗口大小自動適應圖片大小，並且不可手動更改
  namedWindow("Normalized RGBD Depth", WINDOW_AUTOSIZE);
  namedWindow("RGBD Trajectory", WINDOW_AUTOSIZE);
  Mat traj = Mat::zeros(WINDOW_SIZE, WINDOW_SIZE, CV_8UC3);  //3通道每個矩陣元素包含3個uchar值
  ...
  double fontScale = 1;
  int thickness = 1;
  cv::Point textOrg(10, 50);  // 座標為(10, 50)

  Mat rotationMatrix, tranlslationMatrix;

  float vals[] = {FOCUS_LENGTH, 0., CX, 0., FOCUS_LENGTH, CY, 0., 0., 1.};

  const Mat cameraMatrix = Mat(3, 3, CV_32FC1, vals); // rows columns type, Scalar&
